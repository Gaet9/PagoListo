...
async function PerfilSubscripcionesContent({
  ...
  const { canManage, activeNegocioId } = await getSaasAbonoAccessForUser(supabase, auth.user.id);
  if (!canManage) {
    redirect("/perfil");
  }
  ...
  return (
    <PageShell surface="card" padding="md" rounded="lg" className="space-y-4">
      ...
      {subscriptionAllowsCancel(row) ?
        <SuscripcionCancelButton accessUntilLabel={accessUntilLabel} negocioId={activeNegocioId} />
      : null}
