...
/** Recharts v3 `Tooltip` no acepta `className`; usá `wrapperStyle` / `contentStyle` si hace falta. */
...
export function ChartTooltipContent({
    ...
    const resolvedLabel = labelFormatter ? labelFormatter(labelStr) : labelStr;
    ...
    return (
        <div
            ...
            {hideLabel ? null : <div className='text-xs text-muted-foreground'>{resolvedLabel}</div>}
            <div className='grid gap-2'>
                {payload.map((item) => {
                    const key = String(item.dataKey ?? item.name ?? "");
                    const cfg = config[key];
                    const value = typeof item.value === "number" ? item.value.toLocaleString("es-AR") : String(item.value ?? "");

                    return (
                        <div key={key} className='flex items-center justify-between gap-6'>
                            <div className='flex items-center gap-2'>
                                <span
                                    className={cn("shrink-0", indicator === "dot" ? "h-2.5 w-2.5 rounded-full" : "h-2.5 w-4 rounded-sm")}
                                    style={{
                                        background: (cfg?.color ? `var(--color-${key})` : undefined) ?? item.color ?? undefined,
                                    }}
                                />
            ...
            {Object.entries(config)
                ...
                .map(([key, v]) => (
                    <div key={key} className='flex items-center gap-2 text-xs text-muted-foreground'>
                        <span className='h-2.5 w-2.5 rounded-full' style={{ background: v.color ? `var(--color-${key})` : undefined }} />
