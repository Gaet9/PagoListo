...
describe("Accordion", () => {
    it("toggles content", async () => {
        ...
        expect(screen.getByRole("button", { name: "Sección" })).toHaveAttribute("data-state", "open");
        const contentInner = document.querySelector("[data-slot='accordion-content'] > div");
        expect(contentInner).toBeTruthy();
        expect(contentInner).toHaveClass("app-accordion-content-inner");
    });
});
